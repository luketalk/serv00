...
CRON_WEB="nohup /home/${USER}/.vmess/web run -c /home/${USER}/.vmess/config.json >/dev/null 2>&1 &"
CRON_BOT="nohup /home/${USER}/.vmess/bot tunnel --edge-ip-version auto --no-autoupdate --protocol http2 run --token eyJhIjoiZjViNjdhMGJiYTQ4MDIyYjJiNDY5YWMzOTIwOWEzMGYiLCJ0IjoiZDk2Y2RlYjktZGI0ZS00NWUzLWI2MWUtMWQ2MzQyMWQ1ZmZiIiwicyI6Ik1UZ3dabUZqTkRVdE1ESmpZaTAwTURWakxXSm1Namd0WWpkaE56Z3dZekJpTmpRNCJ9 >/dev/null 2>&1 &"
...
if [ "$(command -v pm2)" == "/home/${USER}/.npm-global/bin/pm2" ]; then
  ...
  if [ -e "${WORKDIR}/start.sh" ] && [ -e "${FILE_PATH}/config.json" ]; then
    ...
    (crontab -l | grep -F "@reboot pkill -kill -u $(whoami) && ${CRON_S5} && ${CRON_NEZHA} && ${CRON_BOT} && ${CRON_WEB}") || (crontab -l; echo "@reboot pkill -kill -u $(whoami) && ${CRON_S5} && ${CRON_NEZHA} && ${CRON_BOT} && ${CRON_WEB}") | crontab -
    ...
    (crontab -l | grep -F "* * pgrep -x \"bot\" > /dev/null || ${CRON_BOT}") || (crontab -l; echo "*/12 * * * * pgrep -x \"bot\" > /dev/null || ${CRON_BOT}") | crontab -
    (crontab -l | grep -F "* * pgrep -x \"web\" > /dev/null || ${CRON_WEB}") || (crontab -l; echo "*/12 * * * * pgrep -x \"web\" > /dev/null || ${CRON_WEB}") | crontab -
